...
import {
	MappingItem as BaseMappingItem,
	RawSourceMap,
	SourceMapConsumer,
	SourceMapGenerator,
} from "source-map";
...
export namespace ResolvedJavaScriptMessageBundle {
	...
	export function asTranslatedMessages(
		bundle: ResolvedJavaScriptMessageBundle,
		translatedMessages: Map<string> | undefined,
		problems: string[],
	): string[] {
		const result: string[] = [];
...
class TextModel {
	...
	constructor(
		...
		while ((match = regex.exec(contents))) {
			this.lines.push({
				content: contents.substring(index, match.index),
				ending: match[0],
				mappings: null,
			});
		...
		if (contents.length > 0) {
			this.lines.push({
				content: contents.substring(index, contents.length),
				ending: "",
				mappings: null,
			});
		}
	...
	public apply(patches: Patch[]): void {
		...
		if (patches.length > 1) {
			...
			for (let i = 1; i < patches.length; i++) {
				...
				if (
					previousSpan.end.line > nextSpan.start.line ||
					(previousSpan.end.line === nextSpan.start.line &&
		...
		if (
			lastPatch.span.end.line > this.lines.length ||
			(lastPatch.span.end.line === this.lineCount &&
				lastPatch.span.end.character > lastLine.content!.length)
		) {
			...
			if (this.rawSourceMap) {
				if (startLineNumber === endLineNumber) {
					if (
						...
						mappingCursor.line = startLineNumber;
						mappingCursor.index = startLine.mappings
					...
					while (
						(mappingItem =
							mappingCursor.index !== -1
								? startLine.mappings![mappingCursor.index]
								: null) !== null &&
					...
					if (mappingCursor.index + 1 < startLine.mappings!.length) {
						let mapping =
							startLine.mappings![mappingCursor.index + 1];
						mapping.columnDelta =
							(mapping.columnDelta || 0) + delta;
					}
				} else {
			...
			if (mappings) {
				mappings.forEach((mapping) => {
					...
					sourceMapGenerator.addMapping({
						...
						generated: {
							line: mapping.generatedLine + lineDelta,
							column: mapping.generatedColumn + columnDelta,
						},
					});
				});
			}
...
function analyze(
	...
	function isImportNode(node: ts.Node): boolean {
		if (ts.isImportDeclaration(node)) {
			return (
				...
				vscodeRegExp.test(node.moduleSpecifier.getText())
			);
		}
	...
	const imports = collect(sourceFile, (n) =>
		isRequireImport(n) || isImportNode(n)
	...
	const nlsReferences = imports.reduce<ts.Node[]>((memo, node) => {
		...
		if (ts.isCallExpression(node)) {
			...
			if (
				...
				parent.expression.text === "__importStar"
			) {
	...
	const loadCalls = nlsReferences.reduce<ts.CallExpression[]>(
		(memo, node) => {
							...
							if (node) {
								if (ts.isIdentifier(node)) {
									...
									if (
										...
										errors.push(
											`(${position.line + 1},${position.character + 1}): localize function (bound to ${node.text}) used in an unusual way.`,
										);
									}
								}
							}
		...
		if (loadCall.arguments.length === 0) {
			...
			patches.push({
				span: {
					...
					end: ts.getLineAndCharacterOfPosition(sourceFile, args.end),
				},
				content: relativeFilename
					? `require('path').join(${dir}, '${relativeFilename.replace(/\\/g, "\\\\")}')`
					: "__filename",
			});
		}
		return memo;
		...
		(memo, localizeCall) => {
			...
			memo.patches.push({
				span: {
					start: ts.getLineAndCharacterOfPosition(
						sourceFile,
						firstArg.pos + firstArg.getLeadingTriviaWidth(),
					),
			...
			memo.patches.push({
				span: {
					start: ts.getLineAndCharacterOfPosition(
						sourceFile,
						secondArg.pos + secondArg.getLeadingTriviaWidth(),
					),
					end: ts.getLineAndCharacterOfPosition(
						sourceFile,
						secondArg.end,
...
export function createLocalizedMessages(
	filename: string,
	bundle: ResolvedJavaScriptMessageBundle | PackageJsonMessageBundle,
	...
	const i18nFile =
		(baseDir
			...
			: path.join(i18nBaseDir, languageFolderName, filename)) +
		".i18n.json";
	...
	if (fs.existsSync(i18nFile)) {
		...
		if (Object.keys(messages).length === 0) {
			if (bundleLength > 0) {
				problems.push(
					`Message file ${i18nFile.substr(i18nBaseDir.length + 1)} is empty. Missing messages: ${bundleLength}`,
				);
			}
			messages = undefined;
		}
		...
		if (bundleLength > 0) {
			problems.push(
				`Message file ${i18nFile.substr(i18nBaseDir.length + 1)} not found. Missing messages: ${bundleLength}`,
			);
		}
	}
	...
	if (ResolvedJavaScriptMessageBundle.is(bundle)) {
		translatedMessages =
			ResolvedJavaScriptMessageBundle.asTranslatedMessages(
				bundle,
				messages,
				problems,
			);
	} else {
		translatedMessages = PackageJsonMessageBundle.asTranslatedMessages(
			bundle,
			messages,
			problems,
		);
	...
	if (problems.length > 0) {
		problems.unshift(
			`Generating localized messages for '${languageFolderName}' resulted in the following problems:`,
			"",
		);
		problems.push("", "");
	}
...
export function bundle2keyValuePair(
	bundle: JavaScriptMessageBundle,
	commentSeparator: string | undefined = undefined,
): any {
	...
	for (var i = 0; i < bundle.messages.length; ++i) {
		...
		if (key in result) {
			throw new Error(
				`The following key is duplicated: "${key}". Please use unique keys.`,
			);
		}
...
export class MetaDataBundler {
	...
	add(file: SingleMetaDataFile) {
		this.content[file.filePath.replace(/\\/g, "/")] = {
