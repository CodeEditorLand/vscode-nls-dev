...
import {
	bundle2keyValuePair,
	BundledMetaDataEntry,
	BundledMetaDataFile,
	BundledMetaDataHeader,
	createLocalizedMessages,
	JavaScriptMessageBundle,
	KeyInfo,
	Map,
	MetaDataBundler,
	processFile,
	removePathPrefix,
	resolveMessageBundle,
	SingleMetaDataFile,
} from "./lib";
...
export function rewriteLocalizeCalls(): ThroughStream {
	return through(function (this: ThroughStream, file: FileWithSourceMap) {
		if (!file.isBuffer()) {
		...
		if (result.errors && result.errors.length > 0) {
			...
			if (result.bundle) {
				...
				messagesFile = new File({
					...
					contents: new Buffer(
						JSON.stringify(result.bundle.messages, null, "\t"),
						"utf8",
					),
				});

				let metaDataContent: SingleMetaDataFile = Object.assign(
					{},
					result.bundle,
...
export function bundleMetaDataFiles(id: string, outDir: string): ThroughStream {
	...
	return through(
		function (this: ThroughStream, file: File) {
			...
			if (
				basename.length < NLS_METADATA_JSON.length ||
				NLS_METADATA_JSON !==
					basename.substr(basename.length - NLS_METADATA_JSON.length)
			) {
				this.queue(file);
...
export function createAdditionalLanguageFiles(
	...
	return through(function (this: ThroughStream, file: File) {
		...
		const isAffected =
			isPackageFile || basename.match(/nls.metadata.json$/) !== null;
		...
		const filename = isPackageFile
			...
			: file.relative.substr(
					0,
					file.relative.length - NLS_METADATA_JSON.length,
				);
		...
		if (file.isBuffer()) {
			...
			languages.forEach((language) => {
				...
				const result = createLocalizedMessages(
					filename,
					resolvedBundle,
					folderName,
					i18nBaseDir,
			...
			this.emit(
				"error",
				`Failed to read component file: ${file.relative}`,
			);
	...
	return through(function (this: ThroughStream, file: File) {
		...
		if (file.isBuffer()) {
			...
			if (JavaScriptMessageBundle.is(json)) {
				...
				if (
					resolvedBundle.messages.length !==
					resolvedBundle.keys.length
				) {
					this.queue(file);
				...
				this.emit(
					"error",
					`Not a valid JavaScript message bundle: ${file.relative}`,
				);
			...
			this.emit(
				"error",
				`Failed to read JavaScript message bundle file: ${file.relative}`,
			);
...
export class XLF {
	...
	public addFile(original: string, keys: KeyInfo[], messages: MessageInfo[]) {
		...
		for (let i = 0; i < keys.length; i++) {
			...
			const comment: string | undefined = (function (
			...
			this.files[original].push(
				comment !== undefined
					? { id: key, message: message, comment: comment }
					: { id: key, message: message },
			);
		}
	}
...
export function createXlfFiles(
	projectName: string,
	...
	return through(
		function (this: ThroughStream, file: File) {
			...
			if (basename === "package.nls.json") {
				const json: PackageJsonFormat = JSON.parse(
					buffer.toString("utf8"),
				);
				...
				this.emit(
					"error",
					new Error(
						`${file.path} is not a valid nls or meta data file`,
					),
				);
...
export function prepareJsonFiles(): ThroughStream {
	...
	return through(
		function (this: ThroughStream, xlf: File) {
			let stream = this;
