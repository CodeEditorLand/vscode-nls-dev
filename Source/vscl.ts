...
const argv = yargs
	...
	.option("outDir", {
		alias: "o",
		describe:
			"The output directory. If not specified the files are overwritten in place.",
		string: true,
		demand: false,
	})
	.option("rootDir", {
		alias: "r",
		describe:
			"The root directory of the sources. Only honored when outDir is set.",
		string: true,
		demand: false,
	})
	.option("keepFilenames", {
		describe:
			"Inline filenames when making localization calls. Only honored when rootDir is set.",
		boolean: true,
		demand: false,
	})
	.parseSync();
		...
		matches.forEach((file) => {
			...
			if (sourceMapMatches && sourceMapMatches.length === 2) {
				...
				if (
					...
					sourceMapContent = fs.readFileSync(
						resolvedSourceMapFile,
						"utf8",
					);
				}
			...
			if (result.errors && result.errors.length > 0) {
				...
				if (outDir) {
					if (
						rootDir &&
						resolvedFile.substring(0, rootDir.length) === rootDir
					) {
						outFile = path.join(
							outDir,
							resolvedFile.substring(rootDir.length),
						);
					} else {
